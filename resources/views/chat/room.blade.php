<x-app-layout>
    <div x-data="chatRoom('{{ $room->slug }}')" x-init="init()" class="h-[calc(100vh-8rem)] flex flex-col max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- رأس الغرفة -->
        ...
        <div class="flex flex-1 gap-4 overflow-hidden">
            ...
            <div class="flex-1 flex flex-col min-w-0">
                    ...
                    <template x-for="(message, index) in allMessages" :key="message.id">
                        ...
                        <div x-show="!message.is_system" class="max-w-[70%]">
                            ...
                            <div x-show="!editingMessage || editingMessage.id !== message.id"
                                class="message-bubble rounded-2xl p-3 relative cursor-pointer 
                                        margin-3
                                ...
                                <div x-show="message.reactions_summary && Object.keys(message.reactions_summary).length"
                                    class="absolute -bottom-2 -left-2 bg-white dark:bg-gray-600 rounded-full px-2 py-0.5 text-xs shadow flex gap-1"
                                    x-transition>
                                    ...
                                    <div class="relative group">
                                        <div class="hidden group-hover:block absolute bottom-full mb-1 right-0 bg-gray-800 text-white text-xs rounded-lg px-3 py-1 w-max max-w-[200px] shadow-lg z-10">
                                            <ul class="list-none" x-show="views[message.id] && views[message.id].length > 0">
                                                ...
                                                <li x-show="(views[message.id] || []).length > 5">وآخرون...</li>
                                            </ul>
                                ...
                                <div class="flex flex-col gap-2">
                                    ...
                                    <div class="flex gap-2 items-center flex-wrap">
                                        <button @click="saveEdit()" 
                                                class="px-3 py-1.5 bg-indigo-500 text-white rounded-lg text-sm hover:bg-indigo-600 transition flex items-center gap-1">
                                            <i class="fas fa-save text-xs"></i> حفظ
                                        </button>
                                        <button @click="cancelEdit()" 
                                                class="px-3 py-1.5 bg-gray-200 dark:bg-gray-500 text-gray-700 dark:text-white rounded-lg text-sm hover:bg-gray-300 dark:hover:bg-gray-400 transition">
                                            إلغاء
                                        </button>
                ...
                <div x-show="hiddenMentionCount > 0" 
                    @click="goToNextHiddenMention()"
                ...
                <div class="mt-4 glass rounded-2xl p-2 flex items-end gap-2 relative">
                    ...
                    <div class="flex-1 relative">
                        <!-- قائمة المنشن -->
                        <div x-show="showMentionList && filteredUsers.length > 0" 
                            @click.outside="showMentionList = false"
                            class="absolute bottom-full left-0 right-0 mb-2 bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-h-48 overflow-y-auto z-50 border border-gray-200 dark:border-gray-700"
                            x-transition>
                        ...
                        <textarea x-model="newMessage" 
                            ...
                            @keydown.escape="showMentionList = false"
                            @input="handleMentionInput($event)"
                            placeholder="اكتب رسالتك... @ للمنشن"
                            rows="1" 
                            class="w-full bg-transparent border-none resize-none focus:ring-0 dark:text-white py-2 max-h-32"
                            x-ref="messageInput"></textarea>
                    </div>
                    
                    <button @click="sendMessage()" 
                            :disabled="!newMessage.trim() || sendCooldown || sending"
        ...
        <!-- شريط التفاعلات المنبثق -->
        <div x-show="reactionBar.open" @click.outside="reactionBar.open = false" x-cloak
            ...
            <!-- زر التعديل - فقط لصاحب الرسالة -->
            <button x-show="reactionBar.message?.user_id == {{ Auth::id() }} && canEditMessage(reactionBar.message)"
            ...
            <!-- زر حذف للجميع (فقط لصاحب الرسالة) -->
        ...
        <div x-show="reactionDetail.open" @click.outside="reactionDetail.open = false" x-cloak
             ...
             :style="'top: '+reactionDetail.y+'px; left: '+reactionDetail.x+'px;'"
             x-transition>
            <div class="text-center mb-2">
                <span class="text-3xl" x-text="reactionDetail.emoji"></span>
                <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">
                    <span x-text="reactionDetail.count"></span> تفاعل
                </p>
            </div>
                ...
                <div class="flex-1 overflow-y-auto p-3 grid grid-cols-6 sm:grid-cols-8 gap-2">
                    <template x-for="emoji in filteredEmojis" :key="emoji">
                        <button @click="selectEmoji(emoji, fullEmojiPicker.message)"
                                class="text-2xl hover:scale-125 transition transform rounded-lg hover:bg-white/20 p-1"
                                x-text="emoji"></button>
                    </template>
                </div>
            </div>
    ...
    <style>
        ...
        .member-leave {
            animation: memberLeave 0.5s ease-in forwards;
            overflow: hidden;
        }
        ...
        @keyframes countPulse {
            0% { transform: scale(1); }
            50% { transform: scale(1.3); color: #6366f1; }
        ...
        @keyframes fadeInRestore {
            ...
            100% { opacity: 1; max-height: 200px; margin-bottom: 16px; }
        }
        ...
        .undo-toast {
            ...
            font-family: 'Tajawal', sans-serif; opacity: 0; pointer-events: none;
        }
        ...
        .message-system {
            ...
            color: #4a7c3f; border: 1px solid #c8e6c9;
        }
        .dark .message-system {
            background: linear-gradient(135deg, #1b3a1b, #2d4a2d);
            color: #a5d6a7; border: 1px solid #3e5c3e;
        }
        ...
        .mention-float-btn {
            ...
            transition: all 0.2s;
            animation: mentionFloatIn 0.3s ease-out;
            display: flex;
            align-items: center;
            gap: 6px;
        }
        .mention-float-btn:hover {
            background: #1d4ed8;
            transform: scale(1.05);
            box-shadow: 0 6px 16px rgba(37, 99, 235, 0.5);
        }
        @keyframes mentionFloatIn {
            from { transform: translateY(20px); opacity: 0; }
            to { transform: translateY(0); opacity: 1; }
        }
        .dark .mention-float-btn {
            background: #3b82f6;
        ...
        .dark .message-system-leave {
            background: linear-gradient(135deg, #3d1a1a, #4d1a1a);
            color: #ef9a9a;
...
document.addEventListener('alpine:init', () => {
    Alpine.data('chatRoom', (roomId) => ({
        ...
        mentionCheckTimeout: null,
        seenMentions: [], // ✅ المنشنات اللي شفناها
        ...
        init() {
            ...
            this.joinMessagesAdded = new Set();
            ...
            this.$nextTick(() => {
                ...
                // ✅ نبث مشاهدة كل الرسائل للمستخدمين الآخرين
                this.messages.forEach(msg => {
                    if (msg.user_id != {{ Auth::id() }} && !msg.is_system) {
                        this.markAsViewed(msg.id);
                    }
                });
            });
            ...
            window.Echo.channel(`room.{{ $room->id }}`)
                ...
                .listen('.reaction.toggled', (e) => {
                    ...
                    if (msg) {
                        ...
                        this.updateReactionsSummary(msg);
                    }
                })
                .listen('.message.viewed', (e) => {
                    ...
                    if (msg && msg.user_id == {{ Auth::id() }} && e.userId != {{ Auth::id() }}) {
                        ...
                        const exists = this.views[msg.id].some(v => v.user_id === e.userId);
                        if (!exists) {
                ...
                .listen('.member.joined', (e) => {
                    console.log('Member joined event received:', e);
                    ...
                    const recentDuplicate = this.systemMessages.some(msg => 
                        msg.type === 'member_joined' && 
                        msg.content.includes(e.userName) && 
                    ...
                    if (!recentDuplicate) {
                        this.systemMessages.push({
                            ...
                            user_id: null,
                            content: `لقد انضم ${e.userName} إلى غرفة ${e.roomName}`,
                    ...
                    // إضافة العضو مع أنيميشن
                    const existingMember = this.members.find(m => m.id === e.userId);
                    if (!existingMember) {
                        // إضافة العضو مع كلاس الأنيميشن
                        const newMember = { 
                            id: e.userId, 
                            name: e.userName, 
                ...
                .listen('.member.left', (e) => {
                    console.log('Member left event received:', e);
                    ...
                    if (!recentDuplicate) {
                        this.systemMessages.push({
                            id: 'system-' + Date.now() + '-' + Math.random().toString(36).substr(2, 5),
                            user_id: null,
                            content: `غادر ${e.userName} غرفة ${e.roomName}`,
                            created_at: new Date().toISOString(),
                            is_system: true,
                            type: 'member_left',
                        });
                ...
                .listen('.message.updated', (e) => {
                    console.log('Message updated event received:', e);
                    ...
                    if (msg) {
                        ...
                        msg.is_edited = true;
                        msg.edited_at = e.updatedAt;
                        this.updateAllMessages();
                        
                        // إظهار إشعار
                        this.$dispatch('notify', { 
                            message: 'تم تعديل رسالة', 
                            type: 'info' 
                        });
                    }
                })
            ...
            window.addEventListener('beforeunload', () => {
                navigator.sendBeacon(`/chat/${roomId}/mark-left`, new URLSearchParams({
                    _token: '{{ csrf_token() }}'
                }));
            });
        },
        ...
        // ===== دوال التفاعلات =====
        getTempEmojis(message) { 
            return (message && this.tempEmojisMap[message.id]) || []; 
        },
        
        updateTempEmojis(message, addedEmoji = null, removedEmoji = null) {
            if (!message) return;
            const msgId = message.id;
            if (!this.tempEmojisMap[msgId]) this.tempEmojisMap[msgId] = [];
            const baseSet = new Set(this.baseEmojis);
            if (addedEmoji && !baseSet.has(addedEmoji)) { 
                if (!this.tempEmojisMap[msgId].includes(addedEmoji)) 
                    this.tempEmojisMap[msgId].push(addedEmoji); 
            }
            if (removedEmoji && !baseSet.has(removedEmoji)) { 
                const idx = this.tempEmojisMap[msgId].indexOf(removedEmoji); 
                if (idx > -1) this.tempEmojisMap[msgId].splice(idx, 1); 
            }
        },
        ...
        updateReactionsSummary(message) {
            ...
            message.my_reactions = message.reactions
                .filter(r => r.user_id == {{ Auth::id() }})
                .map(r => r.type);
        },
        
        get filteredEmojis() { 
            return this.emojiData[this.currentEmojiCategory] || []; 
        },
        ...
        // البحث عن المنشنات المخفية للمستخدم الحالي
        checkHiddenMentions() {
            ...
            // نجيب المنشنات اللي شفناها من قبل
            this.seenMentions = JSON.parse(sessionStorage.getItem('seen_mentions_' + roomId) || '[]');
            
            this.hiddenMentions = [];
            
            this.allMessages.forEach(msg => {
                // ✅ تأكد إنها رسالة منشن للمستخدم الحالي وما شافها قبل
                if (!msg.is_system && 
                    msg.content.includes(myName) && 
                    msg.user_id != {{ Auth::id() }} &&
                    !this.seenMentions.includes(msg.id)) {
                    
                    const msgElement = document.querySelector(`[data-message-id="${msg.id}"]`);
                    if (msgElement) {
                        const msgTop = msgElement.offsetTop;
                        
                        // ✅ إذا الرسالة فوق المنطقة الظاهرة (مخفية)
                        if (msgTop < containerTop - 10) {
                            this.hiddenMentions.push({
                                id: msg.id,
                                top: msgTop
                            });
                        } else {
                            // ✅ إذا الرسالة ظاهرة - نحفظها كـ "شوفناها"
                            if (!this.seenMentions.includes(msg.id)) {
                                this.seenMentions.push(msg.id);
                            }
                        }
                    }
                }
        ...
        goToNextHiddenMention() {
            ...
            if (msgElement && container) {
                ...
                setTimeout(() => {
                    msgElement.classList.remove('ring-2', 'ring-blue-400', 'ring-offset-2');
                }, 2000);
            }
    ...
    highlightMentions(text) {
        ...
        // نجيب كل أسماء الأعضاء الموجودين بالقائمة
        const memberNames = this.members.map(m => m.name);
        
        // نعمل escaping للأسماء عشان ما يصير مشاكل بالـ regex
        const escapedNames = memberNames.map(name => 
            name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
        );
        
        // نكون الـ regex من أسماء الأعضاء
        const regex = new RegExp(`(${escapedNames.join('|')})`, 'g');
        
        // نستبدل الأسماء بـ span ملون
        ...
        onScroll(e) { 
            if (e.target.scrollTop === 0 && this.firstMessageId && !this.loading) 
                this.loadMoreMessages();
            
            // تحقق من المنشنات المخفية
            clearTimeout(this.mentionCheckTimeout);
            this.mentionCheckTimeout = setTimeout(() => {
                this.checkHiddenMentions();
            }, 300);
        },
        ...
        async sendMessage() {
            ...
            const payload = { 
                ...
                parent_id: this.replyToMessage?.id || null 
            };
            ...
            try {
                ...
                if (!existingIds.has(data.message.id)) {
                    ...
                    if (this.messages.length === 1) {
                        this.firstMessageId = data.message.id;
                    }
        ...
        selectEmoji(emoji, message) { 
            this.toggleReaction(message, emoji); 
            this.fullEmojiPicker.open = false; 
        },
        ...
        async toggleReaction(message, type) {
            try {
                ...
                if (myReaction) {
                    this.updateTempEmojis(message, myReaction, null);
                } else {
                    const baseSet = new Set(this.baseEmojis); 
        ...
        removeReaction(message, emoji) { 
            this.toggleReaction(message, emoji); 
            this.reactionDetail.open = false; 
        },
        
        heartReaction(message) { 
                    ...
                    setTimeout(() => {
                        ...
                        this.$nextTick(() => { 
                            ...
                            allMessages.forEach((el, idx) => { 
                                if (idx >= deletedIndex) el.classList.add('message-slide-up'); 
                            }); 
                        });
        ...
        hideUndoToast() {
            ...
            if (toast) {
                ...
                setTimeout(() => { this.deletedMessage = null; }, 300);
            }
            if (this.undoTimeout) { 
        ...
        undoDelete() {
            if (this.deletedMessage) {
                ...
                if (insertIndex === -1) {
                    ...
                    this.messages.splice(insertIndex, 0, this.deletedMessage);
                }
                this.hideUndoToast();
        ...
        startEditing(message) {
            if (!this.canEditMessage(message)) {
                ...
                this.$dispatch('notify', { 
                    message: remaining > 0 ? `يمكن التعديل خلال ${remaining} ساعة` : 'انتهى وقت التعديل', 
                    type: 'error' 
                });
                return;
            }
        ...
        async saveEdit() {
            ...
            if (!content) {
                this.editError = 'الرسالة لا يمكن أن تكون فارغة';
                return;
            }
            
            if (content === this.editingMessage.content) {
                this.cancelEdit();
                return;
            }
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(error.error || 'فشل التعديل');
                }
                ...
                if (msg) {
                    ...
                    msg.is_edited = data.message.is_edited;
                ...
                this.editError = error.message || 'حدث خطأ أثناء التعديل';
            }
        },
